...
// Diambil dari Componentry (componentry.dev/docs/components/kinetic-text-reveal).
// Penyesuaian untuk proyek ini:
// - import dari "motion/react" (paket penerus framer-motion).
// - prop `as` agar bisa langsung dirender sebagai <h1>/<h2>; aria-label hanya
//   sekali di elemen induk, semua segmen aria-hidden (sumber aslinya juga
//   menambah span sr-only sehingga teks terbaca dua kali).
// - `data-kinetic` + kelas `kt-seg` untuk fallback <noscript>.

import { cn } from "@/lib/utils";
import { motion, useReducedMotion, type Transition, type Variants } from "motion/react";
import { forwardRef, useEffect, useImperativeHandle, useMemo, useState, type HTMLAttributes } from "react";
...
interface KineticTextRevealProps extends Omit<HTMLAttributes<HTMLElement>, "children"> {
  text: string;
  as?: "span" | "h1" | "h2" | "p";
  className?: string;
  ...
  (
    ...
    const variants: Variants = {
      hidden: shouldReduceMotion
        ? { opacity: 0 }
        : { opacity: 0, x: offset.x, y: offset.y, filter: blur ? "blur(6px)" : "blur(0px)" },
      visible: (index: number) => ({
        ...
        transition: shouldReduceMotion
          ? { duration: 0.01 }
          : { ...transition, delay: getDelay(index, animatedTotal, stagger, staggerFrom) },
      }),
    };
      ...
      >
        {segments.map((segment, index) => {
          ...
          return (
            <span
              key={`${run}-${index}`}
              className={cn("inline-block overflow-hidden align-baseline pb-1", maskClassName)}
              aria-hidden="true"
            >
              <motion.span
                ...
                className={cn("kt-seg inline-block will-change-transform", segmentClassName)}
